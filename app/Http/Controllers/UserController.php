...
class UserController extends Controller
{
    ...
    public function getBlogLikes($blogName)
    {
        $options['limit']  = request()->input('limit', 10);
        $options['offset'] = (request()->input('page', 1) - 1) * $options['limit'];

        try {
            $result = $this->client->getBlogLikes($blogName, $options);

            $collection = collect($result->liked_posts);
            ...
            $blog = $this->client->getBlogInfo($blogName)->blog;

            return view('blog.likes', compact('blog', 'posts'));
