		...
		<div class="mb-4 row">
			<div class="col-12 d-flex justify-content-between">
				<div class="d-flex align-items-center">
					...
					<a href="{{ request()->fullUrlWithQuery(['type' => 'all'])}}" class="border border-blue-700 {{ (request()->type == 'all') ? 'bg-blue-800 text-white' : 'hover:bg-blue-800 hover:text-white text-blue-700' }}  focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">All</a>
					<a href="{{ request()->fullUrlWithQuery(['type' => 'photo'])}}" class="border border-blue-700 {{ (request()->type == 'photo') ? 'bg-blue-800 text-white' : 'hover:bg-blue-800 hover:text-white text-blue-700' }}  focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Photos</a>
					<a href="{{ request()->fullUrlWithQuery(['type' => 'video'])}}" class="border border-blue-700 {{ (request()->type == 'video') ? 'bg-blue-800 text-white' : 'hover:bg-blue-800 hover:text-white text-blue-700' }}  focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Videos</a>
				</div>
			</div>
		</div>
		<div class="container">
			<div class="mx-auto space-y-2 sm:columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-y-6">
				@foreach($posts as $post)
					@if($post->type == 'photo' && request()->type == "all")
					<div class="relative">
					...
					@endif
					@if($post->type == 'video' && request()->type == "all")
					<div class="relative">
						<div class="relative flex-col inline-block w-full min-w-0 break-inside">
					...
					@endif
					@if($post->type == 'photo' && request()->type == "photo")
					<div class="relative">
						<div class="relative flex-col inline-block w-full min-w-0 break-inside">
							<a href="{{ route('post', ['blog' => $post->blog_name ,'id' => $post->id, 'slug' => $post->slug]) }}">
								<img class="object-cover object-center w-full h-full rounded" src="{{ $post->src }}" onerror="this.onerror=null; this.src='images/ohno.png'">
					...
					@endif
					@if($post->type == 'video' && request()->type == "video")
					<div class="relative">
						<div class="relative flex-col inline-block w-full min-w-0 break-inside">
							<a href="{{ route('post', ['blog' => $post->blog_name ,'id' => $post->id, 'slug' => $post->slug]) }}" class="img-c">
								@if(empty($post->src_type) == 'text')
